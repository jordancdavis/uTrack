...
2.1 Basic Data
The system should manage information about POIs (points of interest) of various categories such as restaurants, bookstores, parks, (registered) users and POIs they have visited. It should also store informationabout user opinions and POI ratings. 

POI Data: A unique ID for each POI, name, address, URL, telephone number, year of establishment, hours of operation (treat as a string), price (an estimated expected price per person per visit), keywords, category (restaurant, park, bookstore, etc.). You should add additional information if necessary such as one (or more) picture(s) associated with a POI (but not required).

User Data: For each registered user, you need to maintain: his/her full name, login name, password, address, phone number, POIs he/she has visited and information regarding each visit (such as date, cost as total $ spent, and number of person in the party).

Opinions: Users can provide ‘feedback’ for a POI, as a score (1-10) along with optional short text. Users can also rate other users’ feedback as ‘useless’, ‘useful’, ‘very useful’; finally, they are allowed to declare other users as ‘trusted’ or ‘not-trusted’.
...
2) Visit: After registration, a user can record a visit to any POI (the same user may visit the same POI multiple times). Each user session (meaning each time after a user has logged into the system) may add one or more visits, and all visits added by a user in a user session are reported to him/her for the final review and confirmation, before they are added into the database.
...
• the list of m most expensive POIs (defined by the average cost per head of all visits to a POI) for each category
• the list of m highly rated POIs (defined by the average scores from all feedbacks a POI has received) for each category

14) User awards: At random points of time, the admin user wants to give awards to the ‘best’ users; thus, the admin user needs to know:
...
able to execute various functionality from the web interface. This was achieved via JSP+Tomcat. I used JSP because it allowed me to call the Java classes I created in Phase 2 in an extremely simple fashion.
